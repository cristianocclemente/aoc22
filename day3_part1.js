const getSumPriorities = sacksString => {
    ...
    for(sack of sacks) {
        const firstCompartment = sack.substr(0, sack.length/2)
        const secondCompartment = sack.substr(sack.length/2)
        let sharedItem
        for(item of firstCompartment) if(secondCompartment.includes(item)) sharedItem=item
